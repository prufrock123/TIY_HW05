...
var getAverageTime = function(person) {
	"use strict";
	var allTimes = Object.keys(person),
		val = 0;
	console.log(allTimes);
	
	for (var i=0; i < allTimes.length; i++) {
		val += person[allTimes[i]];
	};

	var average = val/allTimes.length;

	var niceAverage = Math.round(average * 100) / 100;
	return niceAverage;
	// 	var sumTimes += allTimes[i];
	// };
};
